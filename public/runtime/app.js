...
function showJewelry(id,origin){
  ...
  document.getElementById('jewelStyleTag').textContent=item.style==='bazi'?'古法工艺 · 东方禅意':'疗愈水晶 · 现代西方';
  ...
  document.getElementById('jewelBack').textContent=origin==='taro'?'← 返回塔罗解读':origin==='harmony'?'← 返回命局调和':'← 返回命局解读';
  goto('jewel');
}
...
function renderChatMessages(){
  ...
  box.innerHTML=chatHistory.map(m=>
    `<div class="chat-bubble-row ${m.role}"><div class="chat-bubble">${escapeHtml(m.content).replace(/\n/g,'<br>')}</div></div>`
  ).join('');
